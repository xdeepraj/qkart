...
const Checkout = () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setAddresses((prevState) => ({
          all: prevState.all.filter((address) => address._id !== addressId),
          selected: "",
        }));
        enqueueSnackbar("Address deleted succesfully.", {
          variant: "success",
        });
      }
    } catch (e) {
  ...
  const performCheckout = async (token, items, addresses) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const balance = parseInt(localStorage.getItem("balance"), 10);
        const cartValue = getTotalCartValue(items);
        localStorage.setItem("balance", balance - cartValue);
        history.push("/thanks");
      }
    } catch (error) {
      if (error.response) {
  ...
  function handleSelected(addressId) {
  ...
  function handlePlaceOrder() {
    ...
    if (valid) performCheckout(token, items, addresses);
  }
